...
output_folder += '%s_of_%s_%dD_on_%s_%s%d' % (
        fmin.__name__, "dfogn", int(budget_multiplier), suite_name, "trust=", itrr)
...
print('*** benchmarking %s from %s on suite %s ***'
      % (fmin.__name__, "dfogn", suite_name))
time0 = time.time()
